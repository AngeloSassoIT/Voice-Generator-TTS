...
Sito statico pronto per **GitHub Pages**, da caricare manualmente (upload dei file). Nessun build step, nessuna dipendenza esterna.
...
1. Crea un repository su GitHub (es. `voice-generator-tts-site`).
2. Carica **il contenuto** della cartella `Site` nella root del repository (index.html deve stare nella root, non dentro una sottocartella `Site/`).
3. Su GitHub: **Settings → Pages → Source: Deploy from a branch → Branch: main / (root)** → Save.
4. Il sito sarà online su `https://<tuo-username>.github.io/<nome-repo>/`.

## Cose da completare manualmente
...
- Email di supporto già impostata ovunque: **support@simplebuild.it**.
- La lingua scelta dall'utente viene ricordata (localStorage) e applicata su tutte le pagine.
- Per provare il sito in locale basta aprire `index.html` nel browser (doppio clic).
